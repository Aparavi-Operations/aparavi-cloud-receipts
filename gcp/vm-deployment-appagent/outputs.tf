...
##  INSTANCE OUTPUT
 ...
 output "SSH_Connection_bastion" {
     value      = format("ssh connection to instance  ${var.instance_name_bastion} ==> sudo ssh -i ~/.ssh/_aparavi  ${var.admin}@%s",google_compute_instance.aparavi_instance_bastion.network_interface.0.access_config.0.nat_ip)
 }
